...
if (isset($_POST['clicked'])) {
    $nusername = trim($_POST['username']);
    $npassword = trim($_POST['password']); // plain text (simple version)
    $eaddress  = trim($_POST['email']);
    $cnumber   = trim($_POST['contactnumber']);
    ...
    if (strlen($npassword) < 4) {
        ...
        $stmt->bind_param("ss", $nusername, $eaddress);
        $stmt->execute();
        $res = $stmt->get_result();
        if ($res->num_rows > 0) {
            ...
            $ins->bind_param("ssss", $nusername, $npassword, $eaddress, $cnumber);
            if ($ins->execute()) {
